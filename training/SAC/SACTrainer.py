...
class SACTrainer(Trainer):
    def __init__(self, environment, self_play: bool, agent_ids: List[str], state_dim, action_dim, from_save: bool,
        ...
        replay_buffer = SACExperienceReplayBuffer(state_dims=state_dim,action_dims= action_dim, agent_number=len(agent_ids),
                                                              max_size=max_replay_buffer_size, batch_size=batch_size, time_steps=environment.stats.recurrency)
        agent = SACAgent(environment=environment, seed=seed,
                               ...
                               learning_rate=learning_rate, gamma=gamma, tau=tau, alpha=alpha, com_alpha=com_alpha, batch_size=batch_size, model_path=model_path, target_entropy=target_entropy)
        metrics = {
    ...
    def _env_step(self, observation_array, multitimer: Optional[MultiTimer], return_array: np.ndarray):
        (actions_array, new_observation_array, reward_array, done), action_probs = self._agent.act_batched(
            batched_state = observation_array, deterministic=False, env_batcher=self._env_batcher)
    ...
    def learn(self) -> None:
            ...
            actor_metrics = self._agent.train_step_actor(inputs)
            target_shapes = {STATE_KEY: self.extended_shape, ACTION_KEY: (self._batch_size, len(self._agent_ids) * self._environment.stats.action_dimension),
                             REWARD_KEY: (self._batch_size, len(self._agent_ids)), STATE_PRIME_KEY: self.extended_shape, DONE_KEY: (self._batch_size)}
