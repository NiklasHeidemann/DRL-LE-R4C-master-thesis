...
class PPOAgent(Agent):
    ...
    #todo
    def __init__(self, environment_batcher, self_play: bool, agent_ids: List[str], actor_network_generator, critic_network_generator, recurrent: bool, epsilon: float,
                 learning_rate: float, gamma: float, tau: float, alpha: float, com_alpha: float, social_reward_weight:float,
        ...
        self._init(self_play=self_play, agent_ids=agent_ids, actor_network_generator=actor_network_generator,actor_uses_log_probs=True,
                   critic_network_generator=critic_network_generator, recurrent=recurrent, learning_rate=learning_rate, gamma=gamma, tau=tau, alpha=alpha, com_alpha=com_alpha,
    ...
    def train_step_actor(self, batch: Dict[str, tf.Tensor])->Tuple[EarlyStopping, Dict[str,float]]:
        ...
        with tf.GradientTape() as tape:
            ...
            entropy_loss = self._alpha * mov_entropy_loss + self._com_alpha * com_entropy_loss
    ...
    @override
    def train_step_actorrr(self, batch: Dict[str, tf.Tensor])->Tuple[EarlyStopping, Dict[str,float]]:
        if self._self_play:
            log_prob_current,loss,com_entropy_loss, mov_entropy_loss = self._tf_train_step_actor(batch=batch)
            log_ratio = batch[PROB_OLD_KEY] - log_prob_current
            kld = tf.math.reduce_mean((tf.math.exp(log_ratio) - 1) - log_ratio)
            early_stopping = kld > self._kld_threshold
            return early_stopping, {ACTOR_LOSS: loss, ENTROPY: mov_entropy_loss, COM_ENTROPY: com_entropy_loss, KLD: kld}
    ...
    def _compute_actions_one_hot_and_log_prob(self, state, deterministic):
        ...
        actions_one_hot, log_probs, _ = list(zip(*[self._action_sampler.sample_actions(deterministic=deterministic,generator_index=index,
